    ...
    <div className="space-y-4 animate-in fade-in duration-300">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">BullMQ processor workloads, queue status logs and diagnostic triggers</p>
        </div>

        <div className="flex items-center gap-2">
          <button
            onClick={fetchQueueStatus}
            className="p-1.5 bg-white dark:bg-slate-900 border border-slate-205 dark:border-slate-800 rounded-lg text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors"
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-colors flex items-center gap-1.5 ${
              queueStats.isPaused
                ...
                : 'bg-slate-50 dark:bg-slate-950 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-205'
            }`}
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3.5">
        <div className="p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl space-y-1 hover:border-primary/40 transition-colors">
          <span className="text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider block">Active Workers</span>
          <p className="text-xl font-black text-primary">{queueStats.active}</p>
          <div className="text-[8px] text-slate-450 dark:text-slate-500 leading-none">Executing tasks</div>
        </div>

        <div className="p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl space-y-1 hover:border-primary/40 transition-colors">
          <span className="text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider block">Waiting Tasks</span>
          <p className="text-xl font-black text-slate-700 dark:text-slate-350">{queueStats.waiting}</p>
          <div className="text-[8px] text-slate-455 dark:text-slate-500 leading-none">Queued in Redis</div>
        </div>

        <div className="p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl space-y-1 hover:border-primary/40 transition-colors">
          <span className="text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider block">Delayed Tasks</span>
          <p className="text-xl font-black text-slate-700 dark:text-slate-350">{queueStats.delayed}</p>
          <div className="text-[8px] text-slate-455 dark:text-slate-500 leading-none">Schedule triggers</div>
        </div>

        <div className="p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl space-y-1 hover:border-primary/40 transition-colors">
          <span className="text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider block">Completed</span>
          <p className="text-xl font-black text-slate-900 dark:text-white">{queueStats.completed}</p>
          <div className="text-[8px] text-slate-455 dark:text-slate-500 leading-none">Archived jobs</div>
        </div>

        <div className="p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl space-y-1 hover:border-primary/40 transition-colors col-span-2 sm:col-span-1">
          <span className="text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider block">Failed (Bounces)</span>
          <p className="text-xl font-black text-rose-550 dark:text-rose-500">{queueStats.failed}</p>
          <div className="text-[8px] text-slate-455 dark:text-slate-500 leading-none">Retries exhausted</div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 shadow-md space-y-3 transition-colors">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h3 className="text-xs font-bold text-slate-900 dark:text-slate-200">Execution Logs</h3>
            <p className="text-[10px] text-slate-500 dark:text-slate-400">Searchable history of messaging transactions processed by Redis BullMQ workers</p>
          </div>
          
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-850 rounded-lg px-2.5 py-1 w-full sm:w-48">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
              {filteredLogs.length > 0 ? (
                filteredLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-50 dark:hover:bg-slate-950/20 transition-colors text-[11px]">
                    <td className="py-2 px-3 font-mono font-semibold text-slate-400 dark:text-slate-500">{log.id}</td>
                    <td className="py-2 px-3 font-bold text-slate-800 dark:text-slate-200">{log.campaign}</td>
                    <td className="py-2 px-3 font-mono text-slate-700 dark:text-slate-300">{log.recipient}</td>
                    ...
                    <td className="py-2 px-3 text-right text-slate-500">{log.time}</td>
                  </tr>
                ))
              ) : (
                <tr>
