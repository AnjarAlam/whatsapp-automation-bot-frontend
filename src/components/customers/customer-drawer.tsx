...
export default function CustomerDrawer({ isOpen, onClose, onSaveSuccess, customer }: CustomerDrawerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      {/* Slide drawer panel */}
      <div className="relative w-full max-w-md bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 h-full shadow-2xl flex flex-col justify-between z-10 animate-slide-in">
        {/* Header */}
              ...
              <div className="relative">
                <User className="w-4 h-4 absolute left-3.5 top-3 text-slate-400" />
                 <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white focus:outline-none focus:border-primary transition-colors"
                />
              </div>
        ...
        <div className="p-6 border-t border-slate-100 dark:border-slate-800 flex items-center gap-3">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 py-2.5 px-4 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 font-semibold text-slate-600 dark:text-slate-200 text-xs rounded-xl transition-all text-center"
          >
            Cancel
          </button>
          <button
            ...
            className="flex-1 py-2.5 px-4 bg-primary hover:bg-primary-hover font-semibold text-white text-xs rounded-xl transition-all flex items-center justify-center gap-1.5 shadow-md shadow-primary/20 disabled:opacity-50"
          >
            {isSubmitting ? (
